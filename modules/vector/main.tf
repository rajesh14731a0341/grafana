...
# Target Groups
######################
data "aws_lb_target_group" "clickhouse_tg" {
  name = "d3po-clickhouse-tg"
}

data "aws_lb_target_group" "nginx_vector_tg" {
  name = "nginx-tg"
}

data "aws_lb_target_group" "vector_tg" {
  name = "d3po-vector-tg"
}
...
resource "aws_ecs_service" "vector" {
  ...
  load_balancer {
    target_group_arn = data.aws_lb_target_group.vector_tg.arn
...
resource "aws_ecs_service" "clickhouse" {
  ...
  load_balancer {
    target_group_arn = data.aws_lb_target_group.clickhouse_tg.arn
...
resource "aws_ecs_service" "nginx" {
  ...
  load_balancer {
    target_group_arn = data.aws_lb_target_group.nginx_vector_tg.arn
