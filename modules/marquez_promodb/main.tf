...
locals {
  marquez_api_url_base = "http://${data.aws_lb.internal_alb.dns_name}/promodb/api"
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "MARQUEZ_POSTGRES_PORT",     value = var.promodb_marquez_postgres_port },
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "MARQUEZ_URI"
          value = "http://${data.aws_lb.internal_alb.dns_name}/promodb/api/v1/lineage"
        }
      ]
...
resource "aws_appautoscaling_target" "api" {
  ...
  resource_id        = "service/${var.ecs_cluster_name}/promodb-marquez-api"
...
resource "aws_appautoscaling_target" "web" {
  ...
  resource_id        = "service/${var.ecs_cluster_name}/promodb-marquez-web"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
...
resource "aws_appautoscaling_target" "vector" {
  max_capacity       = 2
  min_capacity       = 1
  resource_id        = "service/${var.ecs_cluster_name}/promodb-vector"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
